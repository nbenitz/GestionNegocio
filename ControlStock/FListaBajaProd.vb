...
Public Class FListaBajaProd
        ...
        Select Case Indice
            Case Is = 0 'Todos
                ...
                Select Case IndiceMotivo
                    Case Is = 0 'Todos
                        TablaBaja = Producto.BuscViewBaja("ORDER BY idBaja DESC")
                    Case Else 'Algun Motivo
                        Dim Motivo As String = cmbMotivo.Text
                        TablaBaja = Producto.BuscViewBaja("WHERE Motivo = '" + Motivo + "'  ORDER BY idBaja DESC")
                End Select
                CargarTabla(TablaBaja)
    ...
    Private Sub cmbMotivo_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbMotivo.SelectedIndexChanged, dtpDesde.ValueChanged, dtpHasta.ValueChanged
        ...
        Select Case IndiceFiltrar
            ...
            Case Is = 2 'Fecha
                ...
                Select Case IndiceMotivo
                    Case Is = 0 'Todos
                        TablaBaja = Producto.BuscViewBaja("WHERE Fecha > '" + Fecha1 + "' and Fecha < '" + Fecha2 + "' ORDER BY idBaja DESC")
                    Case Else 'Algun Motivo
                        Dim Motivo As String = cmbMotivo.Text
                        TablaBaja = Producto.BuscViewBaja("WHERE Fecha > '" + Fecha1 + "' and Fecha < '" + Fecha2 + "' AND Motivo = '" + Motivo + "'  ORDER BY idBaja DESC")
                End Select
        End Select
        CargarTabla(TablaBaja)
    End Sub
    ...
    Private Sub CargarTabla(ByVal Tabla As DataTable)
        ...
        If Filas > 0 Then
            For i  As Integer = 0 To (Filas - 1)
                Dim idBaja As Integer = CInt(Tabla.Rows(i).Item(0))
                ...
                DataGridView1.Rows.Add(idBaja, Fecha, idProd, Descrip, Motivo, Cant)
            Next
        End If
    End Sub
    ...
    Private Sub txtCod_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtCod.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            If txtCod.Text <> "" Then
                Try
                    idProd = txtCod.Text
                    TablaBaja = Producto.BuscViewBaja("WHERE idProductoFK = '" + idProd + "'")
                    CargarTabla(TablaBaja)
